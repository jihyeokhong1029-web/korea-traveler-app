...
1. 이 폴더 전체를 새 GitHub 저장소로 올립니다 (예: `korea-traveler-app`).
   - GitHub 웹사이트에서 "New repository" → 저장소 생성 → 이 폴더의 파일들을
     업로드(또는 `git push`)하면 됩니다.
2. 저장소에 이미 `.github/workflows/build-apk.yml` 파일이 포함되어 있어서,
   push 하는 순간 자동으로 빌드가 시작됩니다.
3. GitHub 저장소 페이지 상단의 **Actions** 탭 → 방금 실행된 워크플로 클릭 →
   화면 아래 **Artifacts** 항목에서 `korea-traveler-debug-apk`를 내려받습니다.
   (약 3~5분 정도 걸려요.)
4. 압축을 풀면 `app-debug.apk` 파일이 나옵니다. 이 파일을 안드로이드 기기로
   옮겨서 설치하면 바로 실행할 수 있습니다. (기기 설정에서 "출처를 알 수
   없는 앱 설치 허용"이 필요할 수 있어요.)
